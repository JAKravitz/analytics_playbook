# Disease Forewarning API (`disease-forewarning-api`)

## Document control
- **API ID:** disease-forewarning-api
...
## Spectral uplift
- **MSI baseline:** Coarse disease-risk flags from climate + index anomalies.
- **VNIR HSI adds:** Chlorophyll depletion and carotenoid accumulation patterns consistent with early disease onset (500–570nm, 700–730nm).
- **VNIR-SWIR HSI adds:** Adds lignin/cellulose disruption signatures of fungal infection (2100nm); improved separation of fungal vs. bacterial and viral disease.

## Purpose and scope
Add-on API endpoint composed on top of the Core API base package; sold per-subscription.

**Catalog notes:** Disease pressure and outbreak signals.
...
## Definition
Raster (or agreed vector) layer representing **Disease Forewarning API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Disease Forewarning API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
