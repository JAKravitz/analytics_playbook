# Expected-State API (`expected-state-api`)

## Document control
- **API ID:** expected-state-api
...
## Spectral uplift
- **MSI baseline:** Index-space baselines; sensitive to illumination conditions.
- **VNIR HSI adds:** Baseline expressed in biophysical units (LAI, cab, cw) — more stable across illumination conditions.
- **VNIR-SWIR HSI adds:** Baseline extended to cm and SWIR-derived traits — richer expected state model with lower uncertainty.
...
**Catalog notes:** Expected condition baseline by crop / stage / time window.
...
## Definition
Raster (or agreed vector) layer representing **Expected-State API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Expected-State API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
