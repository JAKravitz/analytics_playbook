# Field Boundary API (`field-boundary-api`)

## Document control
- **API ID:** field-boundary-api
...
**Catalog notes:** Pixxel-managed field polygons and stable field IDs.
...
## Definition
Raster (or agreed vector) layer representing **Field Boundary API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Field Boundary API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
