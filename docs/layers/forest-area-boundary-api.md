# Area Boundary API (`forest-area-boundary-api`)

## Document control
- **API ID:** forest-area-boundary-api
- **Verticals:** forestry
- **Kind:** core
...
## Spectral uplift
- **MSI baseline:** Baseline geometry on MSI and open reference layers.
...
**Catalog notes:** Foundation geometry for all forest analytics.
...
## Definition
Raster (or agreed vector) layer representing **Area Boundary API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Area Boundary API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
