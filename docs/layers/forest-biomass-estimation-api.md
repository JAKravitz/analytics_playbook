# Biomass Estimation API (`forest-biomass-estimation-api`)

## Document control
- **API ID:** forest-biomass-estimation-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Greenness AGB proxy.
- **VNIR HSI adds:** LAI and cab-informed biomass proxy via Structure API — improved over pure MSI greenness.
- **VNIR-SWIR HSI adds:** SWIR-derived cm and lignin from Structure API directly constrain biomass estimation.
...
## Definition
Raster (or agreed vector) layer representing **Biomass Estimation API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Biomass Estimation API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
