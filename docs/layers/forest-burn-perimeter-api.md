# Perimeter Mapping API (`forest-burn-perimeter-api`)

## Document control
- **API ID:** forest-burn-perimeter-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Thermal perimeter.
- **VNIR HSI adds:** Burn perimeter mapped against biochemically-informed pre-fire forest mask — improved edge delineation.
- **VNIR-SWIR HSI adds:** SWIR burn scar discrimination — improved perimeter accuracy in mixed burned/unburned margins.
...
## Definition
Raster (or agreed vector) layer representing **Perimeter Mapping API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Perimeter Mapping API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
