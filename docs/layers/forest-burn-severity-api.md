# Burn Severity API (`forest-burn-severity-api`)

## Document control
- **API ID:** forest-burn-severity-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** dNBR-style severity where available.
- **VNIR HSI adds:** Severity assessed against biochemical pre-fire baseline — cab and cw loss quantified.
- **VNIR-SWIR HSI adds:** SWIR structural signals (cm, lignin loss) add structural severity dimension — more complete severity characterization.
...
## Definition
Raster (or agreed vector) layer representing **Burn Severity API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Burn Severity API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
