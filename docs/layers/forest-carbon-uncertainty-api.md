# Uncertainty API (`forest-carbon-uncertainty-api`)

## Document control
- **API ID:** forest-carbon-uncertainty-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Uncertainty from input variance.
- **VNIR HSI adds:** Uncertainty informed by spectral retrieval quality across VNIR features.
- **VNIR-SWIR HSI adds:** Full-curve spectral fit reduces retrieval uncertainty — tighter Monte Carlo bounds.
...
## Definition
Raster (or agreed vector) layer representing **Uncertainty API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Uncertainty API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
