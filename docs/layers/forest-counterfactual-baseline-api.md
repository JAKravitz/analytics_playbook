# Counterfactual Baseline API (`forest-counterfactual-baseline-api`)

## Document control
- **API ID:** forest-counterfactual-baseline-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Index-based counterfactual.
- **VNIR HSI adds:** Counterfactual expressed in biophysical units — more stable reference than index-based trajectories.
- **VNIR-SWIR HSI adds:** SWIR-extended counterfactual captures structural divergence from intervention areas.
...
## Definition
Raster (or agreed vector) layer representing **Counterfactual Baseline API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Counterfactual Baseline API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
