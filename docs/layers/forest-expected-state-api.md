# Expected-State API (`forest-expected-state-api`)

## Document control
- **API ID:** forest-expected-state-api
- **Verticals:** forestry
- **Kind:** internal
...
## Spectral uplift
- **MSI baseline:** Index-space expected trajectories.
- **VNIR HSI adds:** Baseline expressed in biophysical units (LAI, cab, cw) — more stable across illumination and seasonal conditions.
- **VNIR-SWIR HSI adds:** Baseline extended to SWIR-derived structural traits — richer expected state model capturing canopy water and dry matter dynamics.
