# Fire Corroboration API (`forest-fire-corroboration-api`)

## Document control
- **API ID:** forest-fire-corroboration-api
- **Verticals:** forestry
- **Kind:** core
...
## Spectral uplift
- **MSI baseline:** Thermal and FIRMS-driven corroboration; MSI context only.
- **VNIR HSI adds:** No uplift — fire corroboration is thermal/external data driven.
...
## Definition
Raster (or agreed vector) layer representing **Fire Corroboration API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Fire Corroboration API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
