# Growth Trajectory API (`forest-growth-trajectory-api`)

## Document control
- **API ID:** forest-growth-trajectory-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Progress vs. seasonal index expectation.
- **VNIR HSI adds:** Progress tracked in biophysical units against biochemically-informed expected trajectory.
- **VNIR-SWIR HSI adds:** SWIR structural signals extend trajectory tracking into canopy maturation phase.
...
## Definition
Raster (or agreed vector) layer representing **Growth Trajectory API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Growth Trajectory API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
...
## Operational notes
Indicative list rate in catalog: **$17/km²** (commercial placeholder until finance standardizes). Processing cadence and latency follow the order’s capture plan.
