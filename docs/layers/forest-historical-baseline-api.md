# Historical Baseline API (`forest-historical-baseline-api`)

## Document control
- **API ID:** forest-historical-baseline-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Historical index trends.
- **VNIR HSI adds:** Biochemically-informed historical baseline — more sensitive to gradual historical degradation.
- **VNIR-SWIR HSI adds:** SWIR structural signals improve historical degradation detection — key for REDD+ additionality assessment.
...
## Definition
Raster (or agreed vector) layer representing **Historical Baseline API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Historical Baseline API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
