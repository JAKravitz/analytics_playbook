# Land Cover API (`forest-land-cover-api`)

## Document control
- **API ID:** forest-land-cover-api
- **Verticals:** forestry
- **Kind:** internal
...
## Spectral uplift
- **MSI baseline:** Broadband land cover context.
- **VNIR HSI adds:** Improved separation of spectrally similar land cover classes at forest margins.
- **VNIR-SWIR HSI adds:** Adds discrimination of dry vegetation, NPV, and soil classes via SWIR — reduces misclassification at forest-agriculture boundaries.
...
## Definition
Raster (or agreed vector) layer representing **Land Cover API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Land Cover API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
