# Leakage Analysis API (`forest-leakage-analysis-api`)

## Document control
- **API ID:** forest-leakage-analysis-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Boundary buffer land-cover shift.
- **VNIR HSI adds:** Leakage signals informed by improved land cover discrimination at project boundaries.
- **VNIR-SWIR HSI adds:** SWIR adds NPV and dry vegetation discrimination — more accurate leakage delineation.
...
## Definition
Raster (or agreed vector) layer representing **Leakage Analysis API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Leakage Analysis API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
