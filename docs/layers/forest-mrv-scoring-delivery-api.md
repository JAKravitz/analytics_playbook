# Scoring & Delivery API (`forest-mrv-scoring-delivery-api`)

## Document control
- **API ID:** forest-mrv-scoring-delivery-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** No spectral uplift — scoring is parameter and methodology driven.
...
## Definition
Raster (or agreed vector) layer representing **Scoring & Delivery API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Scoring & Delivery API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
