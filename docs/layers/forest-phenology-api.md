# Phenology API (`forest-phenology-api`)

## Document control
- **API ID:** forest-phenology-api
- **Verticals:** forestry
- **Kind:** internal
...
## Spectral uplift
- **MSI baseline:** NDVI phenology milestones.
- **VNIR HSI adds:** Stage transitions via chlorophyll dynamics — more precise than NDVI inflection alone.
- **VNIR-SWIR HSI adds:** Adds senescence and dry matter accumulation staging via SWIR.
...
## Definition
Raster (or agreed vector) layer representing **Phenology API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
