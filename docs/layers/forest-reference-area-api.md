# Reference Area API (`forest-reference-area-api`)

## Document control
- **API ID:** forest-reference-area-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Geometry-led reference selection.
- **VNIR HSI adds:** Improved reference area delineation via forest type discrimination.
- **VNIR-SWIR HSI adds:** SWIR adds structural forest type matching — more defensible reference area selection.
...
## Definition
Raster (or agreed vector) layer representing **Reference Area API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Reference Area API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
