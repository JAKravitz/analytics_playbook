# Reference Emissions API (`forest-reference-emissions-api`)

## Document control
- **API ID:** forest-reference-emissions-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Emissions from land-cover change counts.
- **VNIR HSI adds:** Emissions baseline informed by biochemically-sensitive historical degradation detection.
- **VNIR-SWIR HSI adds:** SWIR structural signals extend emissions baseline sensitivity to subtle historical degradation.
...
## Definition
Raster (or agreed vector) layer representing **Reference Emissions API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Reference Emissions API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
