# Wildfire Composite Risk API (`forest-wildfire-composite-risk-api`)

## Document control
- **API ID:** forest-wildfire-composite-risk-api
- **Verticals:** forestry
...
## Spectral uplift
- **MSI baseline:** Composite from weather and indices.
- **VNIR HSI adds:** Partial — VNIR-constrained fuel inputs limit composite risk precision.
- **VNIR-SWIR HSI adds:** SWIR-derived fuel moisture and load enable highest-confidence composite risk scoring.
...
## Definition
Raster (or agreed vector) layer representing **Wildfire Composite Risk API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Wildfire Composite Risk API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
