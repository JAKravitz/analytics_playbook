...
**Catalog notes:** Field-relative, not regional
...
## Definition
Raster (or agreed vector) layer representing **Anomaly Score**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Anomaly Score** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
...
## Operational notes
Indicative list rate in catalog: **$25/km²** (commercial placeholder until finance standardizes). Processing cadence and latency follow the order’s capture plan.
