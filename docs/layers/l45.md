...
## Definition
Raster (or agreed vector) layer representing **Ammonia (non-optical model)**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Ammonia (non-optical model)** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
