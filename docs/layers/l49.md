...
**Catalog notes:** Public-health / utility response
...
## Definition
Raster (or agreed vector) layer representing **HAB Risk Zone Mapping**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **HAB Risk Zone Mapping** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
