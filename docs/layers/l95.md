# Change Magnitude + Direction (`l95`)
...
**Catalog notes:** Vector / magnitude change
...
## Definition
Raster (or agreed vector) layer representing **Change Magnitude + Direction**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Change Magnitude + Direction** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
