# Portfolio Aggregation API (`portfolio-aggregation-api`)

## Document control
- **API ID:** portfolio-aggregation-api
...
## Spectral uplift
- **MSI baseline:** Geometry / statistics; sensor-agnostic.
- **VNIR HSI adds:** No uplift — aggregation is geometry and statistics.
- **VNIR-SWIR HSI adds:** No uplift

## Purpose and scope
Aggregate or normalize signals across many fields for portfolio, finance, or insurance-style views.

**Catalog notes:** Risk roll-up across fields and geographies.
...
## Definition
Raster (or agreed vector) layer representing **Portfolio Aggregation API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Portfolio Aggregation API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
