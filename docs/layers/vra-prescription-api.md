# VRA Prescription API (`vra-prescription-api`)

## Document control
- **API ID:** vra-prescription-api
...
## Spectral uplift
- **MSI baseline:** NDVI-driven management zones.
- **VNIR HSI adds:** Sub-field stress zones defined by biochemical gradients — spatially more precise than NDVI-based zones.
- **VNIR-SWIR HSI adds:** Stress zones informed by SWIR-derived water and structural signals — more accurate irrigation and fungicide prescription zones.

## Purpose and scope
Add-on API endpoint composed on top of the Core API base package; sold per-subscription.

**Catalog notes:** Zone-based variable-rate prescriptions for fertilizer, irrigation, or chemicals.
...
## Definition
Raster (or agreed vector) layer representing **VRA Prescription API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **VRA Prescription API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
