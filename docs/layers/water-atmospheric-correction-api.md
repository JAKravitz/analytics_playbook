# Atmospheric Correction / Surface Reflectance API (`water-atmospheric-correction-api`)

## Document control
- **API ID:** water-atmospheric-correction-api
- **Verticals:** water
- **Kind:** internal
...
**Catalog notes:** Internal — available on request at no extra charge.
...
## Definition
Raster (or agreed vector) layer representing **Atmospheric Correction / Surface Reflectance API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Atmospheric Correction / Surface Reflectance API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
