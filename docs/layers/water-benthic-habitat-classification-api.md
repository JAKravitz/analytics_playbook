# Benthic Habitat Classification API (`water-benthic-habitat-classification-api`)

## Document control
- **API ID:** water-benthic-habitat-classification-api
- **Verticals:** water
...
- **VNIR HSI adds:** Full VNIR spectral curve enables species-assemblage level benthic discrimination — MSI broadband conflates bottom classes
- **VNIR-SWIR HSI adds:** SWIR adds no uplift for benthic mapping — SWIR attenuates completely in water column
...
## Definition
Raster (or agreed vector) layer representing **Benthic Habitat Classification API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Benthic Habitat Classification API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
