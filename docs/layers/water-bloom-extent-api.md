# Bloom Extent API (`water-bloom-extent-api`)

## Document control
- **API ID:** water-bloom-extent-api
- **Verticals:** water
...
- **VNIR HSI adds:** Bloom boundary delineated against HSI-informed water mask — more accurate extent in mixed water-vegetation margins
- **VNIR-SWIR HSI adds:** No significant additional uplift for extent mapping
...
## Definition
Raster (or agreed vector) layer representing **Bloom Extent API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Bloom Extent API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
