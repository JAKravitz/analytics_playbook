# Bloom Severity API (`water-bloom-severity-api`)

## Document control
- **API ID:** water-bloom-severity-api
- **Verticals:** water
...
- **VNIR HSI adds:** Severity scored on phycocyanin concentration and Chl-a — spectrally-derived, not proxy-based
- **VNIR-SWIR HSI adds:** SWIR-extended constituent retrievals improve severity scoring in optically complex waters
...
## Definition
Raster (or agreed vector) layer representing **Bloom Severity API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Bloom Severity API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
