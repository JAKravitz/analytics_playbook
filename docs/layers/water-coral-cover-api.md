# Coral Cover API (`water-coral-cover-api`)

## Document control
- **API ID:** water-coral-cover-api
- **Verticals:** water
...
- **VNIR HSI adds:** Coral coverage mapped via spectral separation of coral, algae, and sand bottom classes
...
## Definition
Raster (or agreed vector) layer representing **Coral Cover API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Coral Cover API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
