# Optical Inversion API (`water-optical-inversion-api`)

## Document control
- **API ID:** water-optical-inversion-api
- **Verticals:** water
- **Kind:** core
...
- **VNIR HSI adds:** Full water-leaving reflectance curve — narrow-band features enable constituent-specific absorption decomposition not possible with MSI
- **VNIR-SWIR HSI adds:** SWIR constraints improve water column optical closure — more accurate IOP partitioning
...
## Definition
Raster (or agreed vector) layer representing **Optical Inversion API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Optical Inversion API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
