# Risk Zone API (`water-risk-zone-api`)

## Document control
- **API ID:** water-risk-zone-api
- **Verticals:** water
...
- **VNIR HSI adds:** Risk zones defined by spectrally-confirmed bloom margins — more defensible than index-threshold zones
- **VNIR-SWIR HSI adds:** No significant additional uplift
...
## Definition
Raster (or agreed vector) layer representing **Risk Zone API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Risk Zone API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
