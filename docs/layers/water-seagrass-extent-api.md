# Seagrass Extent API (`water-seagrass-extent-api`)

## Document control
- **API ID:** water-seagrass-extent-api
- **Verticals:** water
...
- **VNIR HSI adds:** Seagrass mapped via narrow-band chlorophyll and accessory pigment features — more accurate extent than MSI
...
## Definition
Raster (or agreed vector) layer representing **Seagrass Extent API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Seagrass Extent API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
