# Surface Water Mask API (`water-surface-water-mask-api`)

## Document control
- **API ID:** water-surface-water-mask-api
- **Verticals:** water
- **Kind:** core
...
- **VNIR HSI adds:** Improved water-land edge delineation via red-edge and NIR water absorption features
- **VNIR-SWIR HSI adds:** No significant additional uplift — water masking well-solved at VNIR
...
## Definition
Raster (or agreed vector) layer representing **Surface Water Mask API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Surface Water Mask API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
