# Threshold / Exceedance API (`water-threshold-exceedance-api`)

## Document control
- **API ID:** water-threshold-exceedance-api
- **Verticals:** water
- **Kind:** core
...
- **VNIR HSI adds:** Earlier exceedance detection via more sensitive constituent retrievals
- **VNIR-SWIR HSI adds:** Higher-confidence exceedances from SWIR-extended retrievals
...
## Definition
Raster (or agreed vector) layer representing **Threshold / Exceedance API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Threshold / Exceedance API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
