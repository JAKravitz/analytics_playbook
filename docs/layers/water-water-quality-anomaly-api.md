# Water Quality Anomaly API (`water-water-quality-anomaly-api`)

## Document control
- **API ID:** water-water-quality-anomaly-api
- **Verticals:** water
- **Kind:** core
...
- **VNIR HSI adds:** Anomalies computed in constituent space — more specific than broadband index anomalies
- **VNIR-SWIR HSI adds:** SWIR-constrained constituents reduce retrieval uncertainty — anomaly detection more sensitive
...
## Definition
Raster (or agreed vector) layer representing **Water Quality Anomaly API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Water Quality Anomaly API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
