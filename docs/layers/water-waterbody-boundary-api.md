# Waterbody Boundary API (`water-waterbody-boundary-api`)

## Document control
- **API ID:** water-waterbody-boundary-api
- **Verticals:** water
- **Kind:** core
...
## Spectral uplift
- **MSI baseline:** Baseline product on MSI imagery.
...
## Definition
Raster (or agreed vector) layer representing **Waterbody Boundary API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Waterbody Boundary API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
