# Webhook Delivery API (`webhook-delivery-api`)

## Document control
- **API ID:** webhook-delivery-api
...
## Spectral uplift
- **MSI baseline:** Standard webhook delivery; payload determined by source API.
- **VNIR HSI adds:** No uplift
- **VNIR-SWIR HSI adds:** No uplift

## Purpose and scope
Add-on API endpoint composed on top of the Core API base package; sold per-subscription.

**Catalog notes:** Push-based delivery of alerts to customer endpoints.
...
## Definition
Raster (or agreed vector) layer representing **Webhook Delivery API**. Exact units, classes, and dynamic range are specified in the delivery metadata for each order.
...
Primary deliverable: **Webhook Delivery API** geospatial layer(s). Auxiliary QA or confidence bands may be included where the product line defines them. Delivery format and tiling follow the standard customer delivery spec for the order.
...
## Operational notes
Indicative list rate in catalog: **$2/km²** (commercial placeholder until finance standardizes). Processing cadence and latency follow the order’s capture plan.
