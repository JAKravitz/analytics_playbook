...
export default function Defense({ state }) {
  const v = seed.verticals.defense;
  const accent = v.color;

  const layers = Array.isArray(state?.layers) && state.layers.length ? state.layers : seedLayers;
  const diRows = useMemo(() => layersForVertical('defense', layers), [layers]);
  const grouped = useMemo(() => groupApiRowsByKind(diRows), [diRows]);
  const addonFamilies = useMemo(() => groupAddonsByFamily(grouped.addon), [grouped.addon]);

  return (
    <>
      <VertHero index={6} name={v.name} sub={v.sub} desc={v.desc} accent={v.color} />

      <Callout type="stop" label="Contract-based — not a SaaS subscription">
        Defense &amp; Intelligence is sold under government / defense procurement, FMS, or OTA
        agreements — <strong>not a standard SaaS subscription</strong>. Pricing is TBD and must go
        through appropriate compliance and export-control review. Do not present SaaS pricing to
        defense prospects without CCO and legal sign-off.
      </Callout>

      <SectionHeader
        label="Base package"
        headline="Site Intelligence Monitor"
        subtitle="Required foundation for all add-ons. Covers persistent change, facility footprint, anomaly detection, and event classification over defined AOIs."
        accent={accent}
      />

      <SectionHeader
        label="Core API"
        subtitle="AOI boundaries, object detection, facility footprint, site anomaly detection, change detection, event classification, confidence, and evidence — the billable foundation of every D&I deployment."
        accent={accent}
      />
      ...
      {grouped.internal.length > 0 && (
        <>
          <SectionHeader
            ...
            subtitle="Surface validity mask, foundational imagery features, and internal change detection signal — bundled on request at no extra charge; not in the standard catalog."
            accent="var(--amber)"
          />
      ...
      {addonFamilies.length > 0 && (
        <>
          <SectionHeader
            ...
            subtitle="Data API for integrators, Mobility &amp; Terrain add-ons, and alerting — all configurable against subscribed Core API outputs."
            accent="var(--purple)"
          />
            ...
            <div key={family} style={{ marginBottom: 18 }}>
              ...
              {family === 'Data API' && (
                <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--gray)', lineHeight: 1.5 }}>
                  Infrastructure Characterization, Material Characterization, and SSE Similarity Search — available to integrators and platform buyers at separate pricing.
                </p>
              )}
              {family === 'Mobility & Terrain' && (
                <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--gray)', lineHeight: 1.5 }}>
                  Soil moisture, soil texture, bearing capacity, terrain classification, trafficability, seasonal mobility windows, and tankability. SWIR (Honeybee) materially improves all outputs; VNIR and MSI tiers are partial capability only.
                </p>
              )}
              {family === 'Alerting' && (
                <p style={{ margin: '0 0 8px', fontSize: 12, color: 'var(--gray)', lineHeight: 1.5 }}>
      ...
      <Callout type="warn" label="Pricing & catalog">
        All D&amp;I rates are placeholders. Contract pricing requires CCO review and export-control
        compliance before any commitment. Defense rows live in{' '}
        <code>src/data/diCatalogRows.js</code>.
      </Callout>
    </>
  );
}
