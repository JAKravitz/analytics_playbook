...
export default function Forestry({ state }) {
  const v = seed.verticals.forestry;
  const accent = v.color;

  const layers = Array.isArray(state?.layers) && state.layers.length ? state.layers : seedLayers;
  const forestRows = useMemo(() => layersForVertical('forestry', layers), [layers]);
  const grouped = useMemo(() => groupApiRowsByKind(forestRows), [forestRows]);
  const addonFamilies = useMemo(() => groupAddonsByFamily(grouped.addon), [grouped.addon]);

  return (
    <>
      <VertHero index={2} name={v.name} sub={v.sub} desc={v.desc} accent={v.color} />

      <Callout type="info" label="API-first revenue model">
        Forestry is sold as a <strong>base package</strong> (Deforestation &amp; Degradation Alerting)
        with a billable <strong>Core API</strong> (including trait and structure inversion) and{' '}
        <strong>internal</strong> supporting layers on request at no extra charge. Add-ons attach on top.
      </Callout>

      <SectionHeader
        label="Base package"
        headline="Deforestation & Degradation Alerting"
        subtitle="Required foundation for forestry add-ons. Every customer starts here."
        accent={accent}
      />

      <SectionHeader
        label="Core API"
        subtitle="AOI identity, forest cover and type, degradation and deforestation detection, fire corroboration, attribution, confidence, evidence, and trait/structure inversion — the billable foundation every customer subscribes to."
        accent={accent}
      />
      ...
      {grouped.internal.length > 0 && (
        <>
          <SectionHeader
            ...
            subtitle="Land cover, vegetation classification, climate forcing, phenology, expected state, anomaly, and change detection — bundled on request at no extra charge; not in the standard catalog."
            accent="var(--amber)"
          />
      ...
      <SectionHeader
        ...
        subtitle="Optional add-on APIs composed on top of the Core API. Families follow the forestry product catalog: ARR &amp; restoration, carbon MRV &amp; REDD+, wildfire, shared recovery monitoring, and alerting."
        accent="var(--purple)"
      />
      {addonFamilies.map(({ family, rows }) => (
        <div key={family} style={{ marginBottom: 18 }}>
          ...
          {family === 'Recovery Monitoring' && (
            ...
            >
              Post-disturbance canopy and structure recovery — shared across wildfire and degradation
              workflows; compose with family-specific event and severity APIs as needed.
            </p>
          )}
          {rows.map((row) => (
      ...
      <Callout type="warn" label="Pricing & catalog">
        Per-API list rates are placeholders for the calculator until commercial signs off. Forestry
        rows live in <code>src/data/forestryCatalogRows.js</code> and merge into{' '}
        <code>src/data/layersCatalog.js</code>; anything tagged with vertical <code>forestry</code>{' '}
