    ...
    <>
      ...
      <Callout type="info" label="North star · NEXUS">
        ...
        <a href="#nexus" style={{ color: 'var(--cyan)' }}>
          Read the NEXUS - NORTH STAR page →
        </a>
      </Callout>
    </>
  );
    ...
    <>
      ...
      <div className="claims-grid">
        <div className="claims-col now">
          <div className="col-head">Now · Operational</div>
          <div>
        ...
        <div className="claims-col eoy">
          <div className="col-head">Being built · 2026+</div>
          <div>
      ...
      <SectionHeader label="Every trait needs a stage" accent={color} />
      <p style={{ maxWidth: 760, fontSize: 14, color: 'var(--gray)' }}>
        A biophysical reading is only interpretable against growth stage — the same chlorophyll or
        water value means different things at tillering and grain fill, so phenology travels with
        every trait. TRACE delivers API layers — traits and indices, not yield numbers or VRA
        prescriptions. SWIR-grade trait separation waits for Honeybee (2027+); operational traits
        today are VNIR / MSI.
      </p>
    </>
  );
...
function MessagingView({ product }) {
  ...
  return (
    <>
      ...
      {messaging.integrationNote && (
        <Callout type="info" label="Integration positioning">
          {messaging.integrationNote}
        </Callout>
      )}
...
function WhitePaperView() {
  return <TraceWhitePaper />;
}

/* ── Root ──────────────────────────────────────────────────────────────── */

export default function AgricultureProduct({ subView }) {
  const product = agricultureProduct;
