...
import { agricultureWhitePaper as wp, agricultureFigures } from '../../data/products/agriculture.js';

const ASSET_BASE = '/product-assets/agriculture/';

function Figure({ slot }) {
  const fig = agricultureFigures[slot];
...
function Section({ section }) {
  return (
    <>
      ...
      {section.traitParameters && (
        <div className="wp-trait-grid">
          {section.traitParameters.map((t) => (
            <div className="wp-trait-card" key={t.id}>
              <div className="wp-trait-id">{t.id}</div>
              <h4>{t.name}</h4>
              <p>{t.body}</p>
            </div>
          ))}
...
export default function TraceWhitePaper() {
  return (
