...
import { waterProduct } from '../../data/products/water.js';
import ProductPricingView from '../../components/ProductPricingView.jsx';
import SwipeWhitePaper from './SwipeWhitePaper.jsx';
...
function ClaimsView({ product }) {
  ...
  return (
    <>
      ...
      <SectionHeader label="Uncertainty as a product" accent={color} />
      <p style={{ maxWidth: 760, fontSize: 14, color: 'var(--gray)' }}>
        Every retrieval ships with a per-pixel confidence bound from the MDN posterior — that is what
        makes SWIPE safe to act on. Non-optical variables (DO, pH, ammonia) are model proxies, never
        direct measurements. PFT species discrimination is roadmap and benthic typing is in
        development; neither is an operational claim. State the grade and the uncertainty on every
        deliverable.
      </p>
    </>
  );
...
function WhitePaperView() {
  return <SwipeWhitePaper />;
}

/* ── Root ──────────────────────────────────────────────────────────────── */

export default function WaterProduct({ subView }) {
  const product = waterProduct;
